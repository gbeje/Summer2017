	...
	for(k=0;k<j;k++){
		if(client.call(IK_service)) //put the request through the server to get response outcome
		{
			if(IK_service.response.isValid[0]==true)
			{
				...
				if(k==0){
					for(i=0;i<7;i++){
						left_limb.names.push_back(IK_service.response.joints[k].name[i]);
						left_limb.command.push_back(IK_service.response.joints[k].position[i]);
					}
				} else {
					for(i=0;i<7;i++){
						left_limb.names[i]=IK_service.response.joints[k].name[i];
						left_limb.command[i]=IK_service.response.joints[k].position[i];
					}
				}
				while(ros::ok){
...
//						ROS_INFO("Slowing down");
			...
			} else if (IK_service.response.isValid[k]==false){
