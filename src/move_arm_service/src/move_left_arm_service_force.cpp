...
geometry_msgs::Vector3 gripper_force;
...
void endpointCallback(const baxter_core_msgs::EndpointState::ConstPtr& endpoint){
	...
	gripper_force = endpoint -> wrench.force;
}
...
//					ROS_INFO("Slowing down");
				...
				if(fabs(current_endpoint.position.x-req.pose.pose.position.x)<0.005 &&
				   ...
				   current_sensor.range < 0.15) {
					...
					break;
					//I don't expect this to work. The command has already been sent (usually) by this point, so not sending it again doesn't actually change anything. 
					//instead, should probably publish a new command of its current location (for the case of the grippers HITTING a surface, not of the IR sensors 
					//detecting a close object, should maybe back away opposite the direction of the force)

					//note: "There are two IR Range sensors on each hand. The sensor data is published on the tf frame <side>_hand_range, and points down the +x axis, 
					//as is convention for rviz" 
				}
				if(fabs(gripper_force.x)>1.0){
					ROS_INFO("hit. stopping");
					break;
				}
			}
...
int main(int argc, char* argv[])
{
	...
	ros::Subscriber endpointSub = n.subscribe("/robot/limb/left/endpoint_state",1000,endpointCallback);
//	posSubPtr=&positionSub;
	ros::Subscriber rangeSub = n.subscribe("/robot/range/left_hand_range/state",1000, rangeCallback);
//	ros::Subscriber forceSub = n.subscribe("/robot/limb/left/endpoint_state",10,forceCallback);
