...
float distances[]={100,100,100,100,100,100,100};
int stamps[]={0,0,0,0,0,0,0};
//# (id) of sensor with current minimum
int min_sensor;
// current minimum distance
float min_distance=100;<F3>
...
void findMin(void){
	int i;
	int found = 0;
	ROS_INFO("last stamp: %d min distance: %f",last_stamp,min_distance);
	//7 is number of sensors whose data is relevant
	for (i=0;i<7;i++){
		ROS_INFO("%d:%f %d",i,distances[i],stamps[i]);
...
//				ROS_INFO("this sensor is the same as sensor of previous minimum");
				min_distance=pc.channels[1].values[i];
				findMin();
				received=0;
			}
